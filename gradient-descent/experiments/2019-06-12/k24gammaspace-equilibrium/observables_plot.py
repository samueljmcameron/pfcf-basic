...
for observable in observable_list:
    ...
    if observable == 'surfacetwist':
        plabels[observable] = r'$\psi(R)$'
    elif observable == 'eta':
        plabels[observable] = r'$2\pi/\eta$'
    elif observable == 'delta':
        plabels[observable] = r'$\delta$'
    elif len(observable) > 1:
        plabels[observable] = fr'$\{observable}$'
    else:
        plabels[observable] = fr'${observable}$'
...
coexist = np.loadtxt(obs.observable_sname("fw_coexist",plot_format="txt"))

for observable in observable_list:
    
    ax[observable].plot(coexist[:,1],coexist[:,0],'k-',lw=3)
    ax[observable].set_xlim(right=0.3)
    fig[observable].subplots_adjust(left=0.2,right=0.95,bottom=0.15)
    fig[observable].suptitle(plabels[observable])
