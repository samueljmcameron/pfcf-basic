...
if __name__=="__main__":
    ...
    k24,gstart,gend = sys.argv[1],float(sys.argv[2]),float(sys.argv[3])

    gamma_0s = np.linspace(0.01,0.4,num=101,endpoint=True)

    num = (gend-gstart)/(gamma_0s[1]-gamma_0s[0])+1
    print(num)
    num = int(round(num))
    print(num)
    gammas = np.linspace(gstart,gend,num=num,endpoint=True)
    print(gammas)

    tmp_path = "../../../tmp_datafwd/"
